...
function StepCheckbox({ checked, onChange }: { checked: boolean; onChange: (v: boolean) => void }) {
  return (
    <button
      ...
      className={cn(
        'w-4 h-4 rounded border-2 transition-all flex items-center justify-center flex-shrink-0 mt-px',
        checked ? 'bg-primary border-primary' : 'border-border-strong hover:border-primary/60',
      )}
    >
      {checked && (
...
export function StepEditModal({ open, onClose, caseItem, onSave }: StepEditModalProps) {
  ...
  return (
    ...
    >
      ...
      {/* Steps */}
      <div>
        {PRODUCTION_STEP_OPTIONS.map((stepOption) => {
          ...
          return (
            ...
            >
              ...
              />
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium text-text mb-1.5">{stepOption.label}</p>
                <Input
